...
describe("First English curriculum", () => {
  it("moves from first words through each module's mission lesson", () => {
    ...
    const moduleThirteenLessons = englishLevelOneLessons.filter(
      (lesson) => lesson.moduleNumber === 13,
    );
    expect(moduleThirteenLessons[0]?.slug).toBe("job-interview");
    expect(moduleThirteenLessons.at(-1)?.slug).toBe("work-mission");

    const moduleFourteenLessons = englishLevelOneLessons.filter(
      (lesson) => lesson.moduleNumber === 14,
    );
    expect(moduleFourteenLessons[0]?.slug).toBe("listening-greetings");
    expect(moduleFourteenLessons.at(-1)?.slug).toBe("listening-mission");

    const moduleFifteenLessons = englishLevelOneLessons.filter(
      (lesson) => lesson.moduleNumber === 15,
    );
    expect(moduleFifteenLessons[0]?.slug).toBe("small-talk-basics");
    expect(moduleFifteenLessons.at(-1)?.slug).toBe("idioms-mission");

    const moduleSixteenLessons = englishLevelOneLessons.filter(
      (lesson) => lesson.moduleNumber === 16,
    );
    ...
    const moduleEighteenLessons = englishLevelOneLessons.filter(
      (lesson) => lesson.moduleNumber === 18,
    );
    expect(moduleEighteenLessons[0]?.slug).toBe("think-picture-description");
    expect(moduleEighteenLessons.at(-1)?.slug).toBe("think-in-english-mission");
  });
